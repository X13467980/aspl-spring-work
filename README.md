...
| **ir_to_txt** | インパルス応答WAVをテキスト（時間 vs 振幅）に変換。gnuplot 用。 |
...
インパルス応答の波形を gnuplot でプロット：
```bash
make plot_ir        # impulse_response_tsp.wav → impulse_response.png
make plot_ir_white  # impulse_response_white.wav → impulse_response_white.png
```
